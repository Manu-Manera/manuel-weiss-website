...
OUTPUT_FILE = HERE / "Knauf_Tempus_Quick_Reference_Guide_PM-RM.docx"
...
SECTIONS: list[str] = [
    ...
    "01_toc.md",
    "02_glossary.md",
    "03_signin_homescreen.md",
    "04_find_open_project.md",
...
def parse_markdown(md_text: str) -> list[Block]:
    ...
    while i < len(lines):
        ...
        # Note / Tip / Warning
        m_typed = _NOTE_PREFIX.match(line)
        if m_typed:
            kind = m_typed.group("kind").lower()
            blocks.append(Block(kind="note", note_kind=kind, text=m_typed.group("text").strip()))
            i += 1
            continue
        m_plain = _NOTE_PLAIN.match(line)
        if m_plain:
            blocks.append(Block(kind="note", note_kind="note", text=m_plain.group("text").strip()))
            i += 1
            continue
        ...
        if _LIST_BULLET.match(line):
            items: list[str] = []
            while i < len(lines):
                m = _LIST_BULLET.match(lines[i].rstrip() or "")
                if not m:
                    break
        ...
        if _LIST_ORDERED.match(line):
            items = []
            while i < len(lines):
                m = _LIST_ORDERED.match(lines[i].rstrip() or "")
                if not m:
                    break
                items.append(m.group(1))
                i += 1
