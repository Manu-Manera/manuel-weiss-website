...
function loadSavedWebsiteData() {
    ...
    for (let i = 1; i <= 3; i++) {
        ...
        if (statValue) {
            ...
            updateElement(`.stat:nth-child(${i}) .stat-number`, statData.value + statData.unit);
        }
        if (statLabel) {
...
function handleAdminUpdate(field, value) {
    console.log('Admin update received:', field, value);
    
    switch (field) {
        case 'hero-name':
            updateElement('.profile-info h1', value);
            break;
        case 'hero-title':
            updateElement('.profile-info h2', value);
            break;
        case 'hero-subtitle':
            updateElement('.hero-subtitle', value);
            break;
        case 'hero-description':
            updateElement('.hero-description', value);
            break;
        case 'contact-name':
            updateElement('.contact-item:nth-child(1) h4', value);
            break;
        case 'contact-title':
            updateElement('.contact-item:nth-child(1) p', value);
            break;
        case 'contact-location':
            updateElement('.contact-item:nth-child(2) p', value);
            break;
        case 'contact-email':
            updateElement('.contact-item:nth-child(3) p', value);
            break;
        case 'contact-phone':
            updateElement('.contact-item:nth-child(4) p', value);
            break;
        default:
            if (field.startsWith('stat-')) {
                const statNum = field.replace('stat-', '');
                const statIndex = parseInt(statNum) - 1;
                if (value && value.value && value.unit) {
                    updateElement(`.stat:nth-child(${statIndex + 1}) .stat-number`, value.value + value.unit);
                }
            } else if (field.startsWith('stat-label-')) {
                const statNum = field.replace('stat-label-', '');
                const statIndex = parseInt(statNum) - 1;
                updateElement(`.stat:nth-child(${statIndex + 1}) .stat-label`, value);
            }
            break;
    }
}
